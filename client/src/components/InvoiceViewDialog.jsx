...
export default function InvoiceViewDialog({ invoice, onClose, onFund }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in zoom-in-95">
      ...
      />
      <div className="relative w-full max-w-2xl bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl flex flex-col divide-y divide-white/10 animate-in fade-in zoom-in-95">
        {/* Header */}
        <div className="flex items-center justify-between p-6">
          <div>
            ...
            <h2 className="text-2xl font-bold text-white leading-tight">Invoice Details</h2>
          </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            <div>
              <span className="text-xs text-white/50 uppercase tracking-wider">MSME Name</span>
              <div className="text-white font-semibold text-lg mt-1">{invoice.msmeName || 'N/A'}</div>
            </div>
            <div>
              <span className="text-xs text-white/50 uppercase tracking-wider">Buyer GSTIN</span>
              <div className="text-white font-semibold mt-1">{invoice.buyerGstin || 'N/A'}</div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="text-white font-semibold text-lg mt-1">₹{Number(invoice.amount).toLocaleString()}</div>
            </div>
            <div>
              <span className="text-xs text-white/50 uppercase tracking-wider">Due Date</span>
              <div className="text-white font-semibold mt-1">{new Date(invoice.dueDate || invoice.createdAt).toLocaleDateString()}</div>
            </div>
            <div>
              <span className="text-xs text-white/50 uppercase tracking-wider">Credit Score</span>
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-xs text-white/50 tabular-nums">{invoice.creditScore}</span>
              </div>
            </div>
            <div>
            ...
            <div>
              <span className="text-xs text-white/50 uppercase tracking-wider">Expected Return</span>
              <div className="text-white font-semibold mt-1">{invoice.expectedReturn ? `${invoice.expectedReturn}%` : 'N/A'}</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between p-6 gap-4">
          ...
          <button
            className="py-3 px-4 rounded-xl bg-white/5 border border-white/10 text-white/70 hover:bg-white/10 hover:text-white transition-colors flex items-center gap-2"
          ...
          <button
            onClick={onClose}
            className="py-3 px-4 rounded-xl bg-white/5 border border-white/10 text-white/70 hover:bg-white/10 hover:text-white transition-colors"
          >
            Close
          </button>
        </div>
